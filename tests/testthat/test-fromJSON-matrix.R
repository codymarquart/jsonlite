...
test_that("fromJSON Matrix", {
  ...
  lapply(objects, function(object){
    newobject <- fromJSON(toJSON1(object));
...
test_that("fromJSON Matrix with simplifyMatrix=FALSE", {
  expect_that(fromJSON(toJSON1(matrix(1)), simplifyMatrix=FALSE), equals(list(1)));
  expect_that(fromJSON(toJSON1(matrix(1)), simplifyVector=FALSE), equals(list(list((1)))));
  expect_that(fromJSON(toJSON1(matrix(NA)), simplifyMatrix=FALSE), equals(list(NA)));
  expect_that(fromJSON(toJSON1(matrix(NA)), simplifyVector=FALSE), equals(list(list((NULL)))));
});
...
test_that("fromJSON Matrix datasets", {
  ...
  lapply(objects, function(object){
    class(object) <- "matrix";
    newobject <- fromJSON(toJSON1(object, digits=4))
    expect_that(newobject, is_a("matrix"));
