...
test_that("escaping and parsing of special characters", {
  ...
  for(i in 1:200){
    x <- createstring(i);
    expect_that(x, equals(fromJSON(toJSON1(x))));
    expect_that(x, equals(fromJSON(toJSON1(x, pretty=TRUE))));
...
test_that("escape solidus", {
  expect_equal(toJSON1("foo/bar/baz"), '["foo/bar/baz"]')
  expect_equal(toJSON1('<script>evil()</script>'), '["<script>evil()<\\/script>"]')
  expect_equal(toJSON1('/', auto_unbox = TRUE), '"/"')
  expect_equal(toJSON1('</', auto_unbox = TRUE), '"<\\/"')

  # Mixed encoding
  x <- c('\xFD\xDD\xD6\xF0\n', '\u1F602\n')
  Encoding(x) <- c("latin1", "UTF-8")
  expect_equal(toJSON1(x), '["\u00FD\u00DD\u00D6\u00F0\\n","\u1F602\\n"]')
